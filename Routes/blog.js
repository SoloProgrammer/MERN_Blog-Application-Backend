...
router.put('/push_reply/:id', fetchuser, async (req, res) => {
    ...
    try {
        ...
        const added_reply = await Blog.updateOne({ "_id": req.params.id, "Comments.comm_id": comm_id }, { $push: { "Comments.$.Replyarr": { user, "reply": rep, rep_id } } })

        const get_blog = await Blog.findById(req.params.id);

        res.json(get_blog);

    } catch (error) {
            res.status(400).json({error:"Some error occured",reason:reportError.message})
    }
})
